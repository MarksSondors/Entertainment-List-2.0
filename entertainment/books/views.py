...
def all_books_page(request):
    """Grid of every book in the database, newest first (mirrors all_movies_page)."""
    books = Book.objects.prefetch_related('authors', 'genres').order_by('-date_added')
    ...
    watchlist_counts = {
        ...
        for item in Watchlist.objects.filter(content_type=book_content_type)
        .values('object_id').annotate(count=Count('user'))
    }

    avg_ratings = {}
    rating_counts = {}
    for item in Review.objects.filter(content_type=book_content_type).values('object_id').annotate(
    ...
    return render(request, 'books/all_books_page.html', context)
